...
class UnemploymentData extends SoaFlux
{
     ...
     * Dans le cas ou aucune date n'est définie, l'on consulte la derniére situation connue.
     * 
     */
    ...
    public function getHTML( $key, $val, $set = null, $trans = null ) 
    {
        ...
        switch($set) {
            case 'date':
                $ret = '<div class="title_flux">'. $key .'</div> ';   
            break;
            
            case 'fieldset': 
                $ret = '<div class="title_flux">'. $key . '</div>
                        <div>'. $this->translate($val) .'</div>';
            break;  
            
            case 'fulldate':
                $ret = '<h2 class="header_center">' . $this->translate($val) . $key . '</h2>';
            break; 
            
            default:  
                $ret = '<div class="recit"><b>' . $this->translate($key). '</b></div> 
 ...
 * Cette fonction permet déobtenir des informations sur le droit au chémage et les paiements réalisés pour une personne donnée. Si une personne n'a pas de droit, la raison de son " non " droit sera donnée.
 * Dans le cas ou la recherche se fait pour une date donnée, ce sont les données du droit et de paiement à cette date qui seront recherchées.
 * Dans le cas ou la recherche se fait pour la derniére situation connue, nous irons d'abord récupérer la derniére situation de paiement connue par l'ONEm et par la suite, nous irons récupérer le droit lié à cette situation de paiement.
 * Date pour laquelle l'on désire effectuer la consultation (AAAA-MM-JJ)
 * Dans le cas ou aucune date n'est définie, l'on consulte la derniére situation connue. 
 * 
 * 1.3.	consultRights
 * Cette fonction permet de consulter la situation du droit au chémage d'une personne. La situation demandée peut étre soit celle à un jour donné, soit la derniére situation connue.
 ...
 * Attention les données ne sont disponibles qu'aprés 6 mois. Cela pour permettre à l'ONEM de fournir des données validées. 
 * 
 * 1.7 consultCareerBreak
 * Cette fonction permet de consulter les informations concernant les interruptions de carriére et/ou crédit-temps déune personne pour une période donnée.
 * 
