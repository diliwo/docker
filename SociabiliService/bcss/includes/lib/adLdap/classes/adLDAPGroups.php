...
class adLDAPGroups {
    ...
    */
    ...
    /**
    * Return a list of computers in a group
    * 
    * @param string $group The group to query
    * @param bool $recursive Recursively get group members
    * @return array
    */
    public function computers($group, $recursive = NULL)
    {
        if (!$this->adldap->getLdapBind()){ return false; }
        if ($recursive === NULL){ $recursive = $this->adldap->getRecursiveGroups(); } // Use the default option if they haven't set it 
        // Search the directory for the members of a group
        ...
        for ($i=0; $i<$users["count"]; $i++){ 
             $filter = "(&(objectCategory=computer)(distinguishedName=" . $this->adldap->utilities()->ldapSlashes($users[$i]) . "))";
             $fields = array("computername", "distinguishedname", "objectClass");
             $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields);
             $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
             ...
             if ($entries['count'] == 0 && $recursive == true) {  
                ...
                $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields);  
                $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);  
                if (!isset($entries[0]['computername'][0])) {
                    continue;  
                }
                $subUsers = $this->members($entries[0]['computername'][0], $recursive);  
                if (is_array($subUsers)) {
                    $userArray = array_merge($userArray, $subUsers); 
                    $userArray = array_unique($userArray);  
                }
             ...
             if ($entries[0]['computername'][0] === NULL && $entries[0]['distinguishedname'][0] !== NULL) {
                 $userArray[] = $entries[0]['distinguishedname'][0];
             }
             elseif ($entries[0]['computername'][0] !== NULL) {
                $userArray[] = $entries[0]['computername'][0];
             }
        }
        return $userArray;
    }
