...
{	
	...
	*/
	private function transformeEnArray()
	{
	...
	*/
	private function rechercherBalises($array_a_traiter,$array_contenu,$nom)
	{
	...
	/**
	* \brief Renvoie l'objet sous forme de tableau.
	* \returns Un Array représentant l'xml original.
	*/
	...
	/**
	* \brief Renvoie l'objet sous forme de XML.
	* \returns La chaine XML originale.
	*/
	...
	* \returns Un Array contenant les résultat trouvés.
	*/
	public function rechercherBalisesDansObjet($nom)
	{
		$array_contenu = array();
		...
		if(is_array($this->array))
		{
			foreach($this->array as $element_a_traiter)
			{
				if( isset( $element_a_traiter["contenu"]) )
				{
					if( is_array( $element_a_traiter["contenu"] ) )
					{					
						$array_a_renvoyer = $this->rechercherBalises($element_a_traiter["contenu"],$array_contenu,$nom);
					}
				}
	...
	*/
	private function analyseContenuBalise($reste)
	{
	...
	*/
	private function recupererProchaineBaliseEtSesAttributs($reste)
	{
		//On trouve la premiere balise a apparaitre
		$position1 = strpos($reste,"<");
		if($position1 === false)
	...
	*/
	private function recupererChaineApresProchaineBalise($reste)
	{
	...
	*/
	private function recupererChaineApresProchainTexte($reste)
	{
		$position = strpos($reste,"<");
	...
	*/
	private function recupererTexte($reste)
	{
		$position = strpos($reste,"<");
	...
	*/
	...
	/**
	* \brief Récupère sous forme de tableau toutes les balises du XML et leur contenu, quel que soit leur position dans l'arborescence elles seront ramenées au premier niveau.
	...
	*/
	public function recupererToutContenuDeArray($array_a_traiter,$array_contenu = array() ) 
	{				
		if(is_array($array_a_traiter))
		{
			foreach($array_a_traiter as $element_a_traiter)
