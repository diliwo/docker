...
{	
	...
	*/
    ...
    /**
	* \brief Transforme en Array simplifié l'array en cours, attention si deux balises ont le meme nom, cela écrasera la balise précédente.
	* \sa recupererProchaineBaliseEtSesAttributs(), recupererChaineApresProchaineBalise(), analyseContenuBalise(), recupererTexte()
	*/
	public function renvoieArraySimple()
	{
        $arraySimple = XmlToArrayForMediPrima::parcourtArraySimple($this->array);
		return $arraySimple;
	}
    
    public static function parcourtArraySimple($element, $debug = 0)
    {
        if($debug)
        {
            echo "<br/>On traite un nouvel élément : ";
            echo "<pre>";
            print_r($element);
            echo "</pre>";
        }
        ...
        //si cet élément ne contient pas d'autres balises
        if(isset($element["contenu"]))
        {
            if($debug)
            {
                echo "<br/>Cet element a une balise contenu.";
            }
            if(!is_array($element["contenu"]))
            {
                if($debug)
                {
                    echo "<br/>La balise contenu contenu n'est pas un array.";
                }
                $array[$element["nom"]] = $element["contenu"];
            }
            else
            {
                if($debug)
                {
                    echo "<br/>La balise contenu est un array.";
                }
                $array[$element["nom"]] = XmlToArrayForMediPrima::parcourtArraySimple($element["contenu"]);
            }
        }
        else
        {
            if($debug)
            {
                echo "<br/>La balise contenu n'est pas trouvée.";
            }
            foreach($element as $sousElement)
            {                
                $sousArray = XmlToArrayForMediPrima::parcourtArraySimple($sousElement);
                // if($debug)
                // {
                    // echo "<br/>L'array est écrasé par : ";
                    // echo "<pre>";
                    // print_r($array);
                    // echo "</pre>";
                // }
                $keys = array_keys($sousArray);
                $key = $keys[0];
                
                $array[$key] = $sousArray[$key];
            }
        }
		...
		if(is_array($array_a_traiter))
		{
			if(isset($array_a_traiter[0]["contenu"]))
			{
				...
				foreach($array_a_traiter as $element_a_traiter)
				{
					if(is_array($element_a_traiter["contenu"]))
					{					
						$array_contenu = XMLToArrayForMediPrima::rechercherBalisesDansArray($element_a_traiter["contenu"],$array_contenu,$nom);
					}
			...
			else
			{
				...
				foreach($array_a_traiter as $key => $contenu)
				{
					if(is_array($contenu))
					{					
						$array_contenu = XMLToArrayForMediPrima::rechercherBalisesDansArray($contenu,$array_contenu,$nom);
					}
		...
		if(is_array($array_a_traiter))
		{
			if(isset($array_a_traiter[0]["contenu"]))
			{
				...
				foreach($array_a_traiter as $element_a_traiter)
				{
					if(is_array($element_a_traiter["contenu"]))
					{					
						$array_contenu = XMLToArrayForMediPrima::PremierDansArray($element_a_traiter["contenu"],$array_contenu,$nom,$niveau);
					}
			...
			else
			{
				...
				foreach($array_a_traiter as $key => $contenu)
				{
					if(is_array($contenu))
					{					
						$array_contenu = XMLToArrayForMediPrima::PremierDansArray($contenu,$array_contenu,$nom,$niveau);
					}
	...
	*/
	...
	{		
		...
		XMLToArrayForMediPrima::staticPrintInHTML($array,-1,false);	
		echo "</div>";
	}
	...
	*/
	private static function staticPrintInHTML($array_a_traiter,$niveau,$afficherNiveaux)
	{
		...
		{			
			...
			for($a = 0; $a < $size; $a++)
			{					
				echo  XMLToArrayForMediPrima::genereEspace($niveau,$afficherNiveaux)."<span style='color:red;'>&lt;".$keys[$a];
					
				echo "&gt;</span>";
				if(is_array($array_a_traiter[$keys[$a]]) && !empty($array_a_traiter[$keys[$a]]))
				{		
					echo "<br />";
					XMLToArrayForMediPrima::staticPrintInHTML($array_a_traiter[$keys[$a]],$niveau,$afficherNiveaux);
					echo XMLToArrayForMediPrima::staticGenereEspace($niveau,$afficherNiveaux);
				}
				else
				{						
	...
	*/
	...
	public static function transformerArrayEnXml($array)
	{
		...
		if( @array_key_exists("contenu", $array) )
		{
			if( is_array( $array["contenu"] ) )
			{
				...
				$xml .= ">".XMLToArrayForMediPrima::transformerArrayEnXml($array["contenu"])."</".$array["nom"].">";
			}
			else
			{
				$xml.= "<".$array["nom"];
			...
			{				
				...
				if( @array_key_exists("contenu", $element_a_traiter) )
				{
					if( is_array( $element_a_traiter["contenu"] ) )
					{
						...
						$xml .= ">".XMLToArrayForMediPrima::transformerArrayEnXml($element_a_traiter["contenu"])."</".$element_a_traiter["nom"].">";
					}
					else
					{
						$xml.= "<".$element_a_traiter["nom"];
	...
	public function nouvelXMLToArrayDepuisBalise($nom, $numero = -1)
	{
		...
		$xml = XMLToArrayForMediPrima::transformerArrayEnXml($temp);
		$temp = new XMLToArrayForMediPrima($xml);
		return $temp;
	}
}
