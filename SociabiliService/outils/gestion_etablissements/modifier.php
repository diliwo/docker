...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['active'])) {
    $sqlt = "UPDATE SSC_ETABLISSEMENT SET ACTIVE = :active WHERE ID_ETABLISSEMENT = :id_etablissement";
    ...
    $values['etablissement'] = $row['ETABLISSEMENT'];
    $values['active'] = $row['ACTIVE'];
    $values['type'] = $row['ID_ETABLISSEMENT_TYPE'];
    $values['rue'] = $row['RUE'];
    ...
    if (empty($values['etablissement']) || empty($values['rue']) || empty($values['numero']) || empty($values['id_code_postaux']) || empty($values['type'])) {
        ...
        $sqlt = "UPDATE SSC_ETABLISSEMENT SET ETABLISSEMENT = :etablissement, ID_ETABLISSEMENT_TYPE = :id_etablissement_type, ACTIVE = :active WHERE ID_ETABLISSEMENT = :id_etablissement";
        $stid = oci_parse($conn, $sqlt);
        ...
        $sqlt = "UPDATE SSC_ADRESSE SET RUE = :rue, NUMERO = :numero, BOITE = :boite, ID_CODE_POSTAUX = :id_code_postaux WHERE ID_ADRESSE = (SELECT ID_ADRESSE FROM SSC_ETABLISSEMENT WHERE ID_ETABLISSEMENT = :id_etablissement)";
        $stid = oci_parse($conn, $sqlt);
...
<head>
    <meta charset="UTF-8">
    <title>Modifier un établissement | Gestion des établissements (Helpdesk Social)</title>
    ...
    <div class="container">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">Modifier un établissement</h3>
            </div>
                        ...
                        <div class="col-lg-11">
                            <select name="type" id="frmFieldType" class="form-control">
                                <?php foreach ($types as $type): ?>
                                    <option value="<?php echo $type['ID_ETABLISSEMENT_TYPE']; ?>" 
                                    <?php 
                                        if(isset($values['type']))
                                        {
                                            if ($values['type'] == $type['ID_ETABLISSEMENT_TYPE'])
                                            {                                            
                                                echo 'selected'; 
                                            }
                                        }                                        
                        ...
                        <div class="col-lg-11">
                            <select name="id_code_postaux" id="frmFieldLocalite" class="form-control">
                                <?php foreach ($codes_postaux as $code_postal): ?>
                                    <option value="<?php echo $code_postal['ID_CODE_POSTAUX']; ?>" 
                                    <?php 
                                        if(isset($values['id_code_postaux']))
                                        {
                                            if ($code_postal['ID_CODE_POSTAUX'] == $values['id_code_postaux'])
                                            {
                                                echo 'selected';
                                            } 
                                        }
