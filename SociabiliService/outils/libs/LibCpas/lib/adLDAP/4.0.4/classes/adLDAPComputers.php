...
class adLDAPComputers {
    ...
    */
	...
	/**
    * Get all computers. Returned in a raw array format from AD
    * 
    * @param string $computerName The name of the computer
    * @param array $fields Attributes to return
    * @return array
    */
    public function getComputerByDesc($description)
    {       
        if (!$this->adldap->getLdapBind()) { return false; }

        $filter = "(&(objectClass=computer)(description=".$description."))";

        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, array("cn","description"));
        $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
    ...
    /**
    * Get all computers from a group. Returned in a raw array format from AD
    * 
    * @param $group OU name.
    * @return array
	* @author Thomas Favay
    */
    public function getComputerByGroup($group)
    {       
 		if (!$this->adldap->getLdapBind()) { return false; }

        $filter = "(objectClass=computer)";
		
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, array("cn","description","memberof"));
        $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
		...
		{
			if(strpos($entry["memberof"][0], $group))
			{
				$resultat[] = $entry;
			}
		}
