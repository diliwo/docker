    ...
    */
    public function members($group, $recursive = NULL)
    {
		...
		{ 
			 ...
			 {
				$temp = array();
				$temp["name"] = $entries[0]['distinguishedname'][0];
				$temp["dn"] = substr($entries[0]['dn'],strpos($entries[0]['dn'],",")+1);
				$temp["mail"] = $entries[0]['userprincipalname'][0];
				$userArray[] = $temp;
             }
             else if ($entries[0]['samaccountname'][0] !== NULL) 
			 {				
				$temp = array();
				$temp["name"] = $entries[0]['samaccountname'][0];
				$temp["dn"] = substr($entries[0]['dn'],strpos($entries[0]['dn'],",")+1);
				$temp["mail"] = $entries[0]['userprincipalname'][0];
				$userArray[] = $temp;
             }
