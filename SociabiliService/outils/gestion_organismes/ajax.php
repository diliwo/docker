...
$sqlt_where_softdelete = 'SSC_ORGANISME.SOFTDELETE = 0';
array_push($sqlt_where_parts, $sqlt_where_softdelete);

if (isset($_GET['libelle']) && !empty($_GET['libelle'])) {
    $sqlt_where_libelle = "LOWER(ORGANISME) LIKE LOWER('%" . $_GET['libelle'] . "%')";
...
if (isset($_GET['type']) && !empty($_GET['type'])) {
    ...
    $sqlt_where_type = 'ID_ORGANISME_TYPE IN (' . substr_replace($sqlt_where_type, '', -1) . ')';
    array_push($sqlt_where_parts, $sqlt_where_type);
} else {
    $sqlt_where_type = 'ID_ORGANISME_TYPE IS NULL';
    array_push($sqlt_where_parts, $sqlt_where_type);
}  
...
if (isset($_GET['active']) && !empty($_GET['active'])) {
    ...
    $sqlt_where_active = 'SSC_ORGANISME.ACTIVE IN (' . substr_replace($sqlt_where_active, '', -1) . ')';
    array_push($sqlt_where_parts, $sqlt_where_active);
} else {
    $sqlt_where_active = 'SSC_ORGANISME.ACTIVE IS NULL';
    array_push($sqlt_where_parts, $sqlt_where_active);
}
...
if (isset($_GET['trier']) && !empty($_GET['trier'])) {
    switch($_GET['trier']) {
        case 'libelle': $sql_orderBy = 'SSC_ORGANISME.ORGANISME'; break;
        case 'type': $sql_orderBy = 'SSC_ORGANISME_TYPE.LIBELLE'; break;
...
$sqlt = "
    SELECT *
    FROM SSC_ORGANISME
    INNER JOIN SSC_ORGANISME_TYPE USING (ID_ORGANISME_TYPE)
...
while (($row = oci_fetch_assoc($stid))) {
    ...
    $item['id'] = $row['ID_ORGANISME'];
    $item['libelle'] = $row['ORGANISME'];
