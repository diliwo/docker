...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sqlt = "UPDATE SSC_ORGANISME SET SOFTDELETE = 1 WHERE ID_ORGANISME = :id_organisme";
    $stid = oci_parse($conn, $sqlt);
    oci_bind_by_name($stid, ':id_organisme',  $_GET['id']);
    oci_execute($stid);
    oci_free_statement($stid);
...
<head>
    <meta charset="UTF-8">
    <title>Supprimer un organisme | Gestion des organismes (Helpdesk Social)</title>
    ...
    <div class="container">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">Supprimer un organisme</h3>
            </div>
            <div class="panel-body text-center">
                <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="POST" class="form-horizontal">
                    <p><strong>Voulez-vous vraiment supprimer l'organisme suivant ?</strong></p>
                    <p>
                        <?php echo $row['ORGANISME']; ?><br>
                        <?php
                        if (!empty($row['RUE'])) {
