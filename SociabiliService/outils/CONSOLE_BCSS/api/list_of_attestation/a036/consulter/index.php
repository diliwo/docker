...
if (isset($_GET["registre_national"])) {
	...
	if ($res->echo == "test") {
		...
		try {
			...
			$arguments .
				"<ServiceRequest>" .
					"<ServiceId>OCMWCPASL036</ServiceId>" .
					"<Version>20060315</Version>" .
					"<ns1:L036RequestReply xmlns:ns1='http://www.ksz-bcss.fgov.be/XSD/SSDN/OCMW_CPAS/L036'>" .
						"<ns1:L036 processType='L'/>" .
                        "<ns1:UniqueReference>" . $reference . "</ns1:UniqueReference>" .
                        "<ns1:INSS>" . $rn . "</ns1:INSS>" .
						"<ns1:Period>" .
							"<ns1:BeginDate>" . $dateDebut->format("Y-m-d") . "</ns1:BeginDate>" .
							"<ns1:EndDate>" . $dateFin->format("Y-m-d") . "</ns1:EndDate>" .
						"</ns1:Period>" .
                        "<ns1:AuthorizedUser>" .
                                "<UserID>".$_config["flux"]["old"]["user_id"]."</UserID>" . 
                                "<Email>".$_config["flux"]["old"]["email"]."</Email>" . 
			...
			try {
				...
				if ($codeErreur > 1) {
					...
					//$listeAttestationsXml = $simpleXml->ServiceReply->children("ns3", true)->L036RequestReply->L036->AnswerOK;

					$retour['data']['a036'] = $simpleXml->ServiceReply->children("ns3", true)->L036RequestReply;
				}
