...
if(isset($_GET["registre_national"]) ) {
    ...
    $pattern = '/([12][0-9]{3})([0-9]{1})$/';
    preg_match($pattern, (isset($_GET['date_debut']) ? $_GET['date_debut'] : NULL) , $beginQuarter );
    preg_match($pattern, (isset($_GET['date_fin']) ? $_GET['date_fin'] : NULL),  $endQuarter);
    
    $firstFormDate = NULL;
    if( is_array( $beginQuarter ) && sizeof( $beginQuarter ) > 0) 
        $firstFormDate = $beginQuarter[1].'Q'.$beginQuarter[2];
        
    $secondFormDate = NULL;
    if( is_array( $endQuarter ) && sizeof( $endQuarter ) > 0)
        $secondFormDate = $endQuarter[1].'Q'.$endQuarter[2];
    
    $now = new DateTime();
    $now->setTimezone(new DateTimeZone("Europe/Brussels"));
        
    $wsdl = "http://" . $_SERVER["HTTP_HOST"] . str_replace("index.php", "", $_SERVER["PHP_SELF"]) . "../../../../../include/soap/self_employed/v2/".$env."/SelfEmployedV2.wsdl";
    ...
    $arguments = array(
        ...
        "criteria" => array(
            "ssin" => $_GET["registre_national"],
             "period"=> array(
                 "beginQuarter" => $firstFormDate
            )
        )
    );
    
    if( !is_null($secondFormDate) )
        $arguments['criteria']['period']['endQuarter'] = $secondFormDate;
