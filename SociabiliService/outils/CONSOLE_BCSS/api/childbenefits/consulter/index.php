...
if (isset($_GET["registre_national"])) {
    ...
    if(is_array($beginDate) && sizeof($beginDate) > 0)
        $firstFormDate = $_GET['date_debut'];

    $secondFormDate = NULL;
    if(is_array($endDate) && sizeof($endDate) > 0)
        $secondFormDate = $_GET['date_fin'];
        

    $now = new DateTime();
    $now->setTimezone(new DateTimeZone("Europe/Brussels"));
    
    $wsdl = "http://" . $_SERVER["HTTP_HOST"] . str_replace("index.php", "", $_SERVER["PHP_SELF"]) . "../../../include/soap/childbenefits/".$env."/be/fgov/kszbcss/intf/ChildBenefitsService/ChildBenefitsV1.wsdl";
    ...
    $arguments = array(
        ...
        "criteria" => array(
            ...
            "period" => array(
                "beginMonth" => (
                    (!is_null( $firstFormDate ) ) ? $firstFormDate : date("Y-m")
                )
            ),
            "authenticSources"=> array(
                "authenticSource" => ( isset($_GET['source']) ? $_GET['source'] : 'WALLONIA' )
            )
        )
    );
    
    if( !is_null($secondFormDate) ) 
        $arguments['criteria']['period']['endMonth'] = $secondFormDate ;
    ...
    try {
        ...
        $retour["data"] = $soapClient->consultFilesBySsin($arguments);
