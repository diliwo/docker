...
$("#transaction25 form").validate({
	...
	submitHandler: function(form) {
		...
		if (RegistreNational.valider( rn )) {
			var url = "<?php echo $_CONFIG['url']; ?>api/transaction25/consulter?env=<?php echo $_SESSION['test_flux_bcss']['env']; ?>&registre_national=" + rn + "&langue=" + langue + "&historicite=" + historicite;
			var jqxhr = $.getJSON( url , function(json) {
				...
				if (typeof json.data.rrn_it_implicit == "undefined") {
					var reponse =
					$("<fieldset />", { 
					...
					reponse.append( JsonHuman.format( json.data.rrn_it_implicit ) );
				}
