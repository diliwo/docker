...
<div class="tab-content well">
	...
	<div class="tab-pane fade in" id="contributions">
		<form id="form-contributions" class="form-horizontal" role="form">
			...
			<div class="form-group">
				...
				<div class="col-md-2">
					<div class="input-group">
					<span class="input-group-addon">Quadrimestre </span>
	...
	<div class="tab-pane fade in" id="carriere_contributions">
		<form id="form-carriere_contributions" class="form-horizontal" role="form">
			<div class="form-group has-feedback">
				<label for="registre_national" class="col-md-2 control-label">Registre national: </label>
				<div class="col-md-3">
					<input type="text" class="form-control" name="registre_national">
...
$("#carriere form").validate({
	...
	submitHandler: function(form) {
		...
		if (RegistreNational.valider( rn )) {
			var url = "<?php echo $_CONFIG['url']; ?>api/self_employed/v1/consulter/carriere?env=<?php echo $_SESSION['test_flux_bcss']['env']; ?>&registre_national=" + rn;
			if ( dateDebut != "" ) {
...
// contributions //
// ************* //
$("#contributions form input[name=q_date_debut],input[name=q_date_fin]").datepicker({
	format: "yyyy",
	language: "fr",
	orientation: "top left",
	minViewMode: 2,
...
$("#contributions form").validate({
	...
	submitHandler: function(form) {
		...
		var q_date_debut = $("#contributions form input[name=q_date_debut]").val();
		
		var fin_quadr = $("#contributions form select[name=fin_quadr]").val();
		var q_date_fin = $("#contributions form input[name=q_date_fin]").val();
		
		if (RegistreNational.valider( rn )) {
			var url = "<?php echo $_CONFIG['url']; ?>api/self_employed/v1/consulter/contributions?env=<?php echo $_SESSION['test_flux_bcss']['env']; ?>&registre_national=" + rn;

			url += "&debut_quadr="+debut_quadr+"&q_date_debut="+q_date_debut;
			url += "&fin_quadr="+fin_quadr+"&q_date_fin="+q_date_fin;
...
$("#carriere_contributions form input[name=q_date_debut], input[name=q_date_fin] ").datepicker({
	format: "yyyy",
	language: "fr",
	orientation: "top left",
	minViewMode: 2,
...
$("#carriere_contributions form").validate({
	...
	submitHandler: function(form) {
		...
		var debut_quadr =  $("#carriere_contributions form select[name=debut_quadr]").val();
		var q_date_debut = $("#carriere_contributions form input[name=q_date_debut]").val();
		
		var fin_quadr =  $("#carriere_contributions form select[name=fin_quadr]").val();
		var q_date_fin = $("#carriere_contributions form input[name=q_date_fin]").val();

		if (RegistreNational.valider( rn )) {
	
			var url = "<?php echo $_CONFIG['url']; ?>api/self_employed/v1/consulter/carriere_contributions?env=<?php echo $_SESSION['test_flux_bcss']['env']; ?>&registre_national=" + rn;
			...
			var jqxhr = $.getJSON( url , function(json) {
				...
				$("#responsejson").getResponsejson(json,'consultCareerAndContributions');
