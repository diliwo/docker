...
$("#situation form").validate({
	...
	submitHandler: function(form) {
		...
		if (RegistreNational.valider( rn )) {
			var url = "<?php echo $_CONFIG['url']; ?>api/unemployment_data/v1/consulter/situation?env=<?php echo $_SESSION['test_flux_bcss']['env']; ?>&registre_national=" + rn;
			if ( date != '' ) {
				url += "&date=" + date;
			}
			...
			var jqxhr = $.getJSON( url , function(json) {
				...
				$("#responsejson").getResponsejson(json,'getSituation'); 
...
$("#sommes_payees form").validate({
	...
	submitHandler: function(form) {
		...
		if (RegistreNational.valider( rn )) {
			var url = "<?php echo $_CONFIG['url']; ?>api/unemployment_data/v1/consulter/sommes_payees?env=<?php echo $_SESSION['test_flux_bcss']['env']; ?>&registre_national=" + rn;
			...
			var jqxhr = $.getJSON( url , function(json) { 
				...
				$("#responsejson").getResponsejson(json,'getPayedSums');
				...
				if(json.succes != 0 )  {
					...
					var data = (json.data.hasOwnProperty('payedSums') ? json.data.payedSums : "" );
...
$("#sommes_payees_activation form").validate({
	...
	submitHandler: function(form) {
		...
		if (RegistreNational.valider( rn )) {
			var url = "<?php echo $_CONFIG['url']; ?>api/unemployment_data/v1/consulter/sommes_payees_activation?env=<?php echo $_SESSION['test_flux_bcss']['env']; ?>&registre_national=" + rn + "&quadrimestre=" + quadrimestre + "&annee=" + annee;

			var jqxhr = $.getJSON( url , function(json) { 

				$("#responsejson").getResponsejson(json,'getPayedActivationSums');
				...
				if(json.succes != 0 )  {
					...
					var data = (json.data.hasOwnProperty('payedActivationSums') ? json.data.payedActivationSums : "" );
